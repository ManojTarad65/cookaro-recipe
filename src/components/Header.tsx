...
export default function Header() {
  ...
  // Public routes (NO HEADER before login)
  const PUBLIC_ROUTES = ["/login", "/register"];

  // 1️⃣ Hide Header on login/register
  if (PUBLIC_ROUTES.includes(pathname)) return null;

  // 2️⃣ Hide Header while session is loading
  if (status === "loading") return null;

  // 3️⃣ If user NOT logged in → Hide Header everywhere except home("/")
  if (!session && pathname !== "/") return null;

  // 4️⃣ If user logged in → Show header on ALL pages including "/"
